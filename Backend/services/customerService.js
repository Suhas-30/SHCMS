...
const insertCustomer = async (customer) => {
    ...
    try {
        ...
        // Insert into CUSTOMER table without CAR_ID
        const custoQuery = 'INSERT INTO CUSTOMER(C_ID, C_NAME, PH, EMAIL, PASSWORD) VALUES(?, ?, ?, ?, ?)';
        const custoValues = [id, name, phone, email, hashedPassword];
        await connection.execute(custoQuery, custoValues);

        // Insert into C_ADDRESS table
